...
private:
	...
	void expand() { // o(n) time complexity
...
public:
	...
	void push_back(type val) { // o(1) amortized time complexity
		if (full()) // if the queue is full we will expand it
			expand(); // o(n) time complexity
	...
	void push_front(type val) { // o(n) time complexity 
		if (full()) // if the queue is full we will expand it
			expand(); // o(n) time complexity
		else if (_front != 0)
			_front--;
		...
		{
			for (int i = capacity; i > 0; i--) // o(n) time complexity
				arr[i] = arr[i - 1];
		}
		arr[_front] = val;
		capacity++;
